...
function onTrackedVideoFrame(video, currentTime, duration) {
    console.log(video.src, currentTime, duration);
    //console.log("Hellow!");
...
function DownloadASX() {
    ...
    $("#durLabel").html('Please enter video duration in seconds from the current time in seconds.' +
        '<br/>Acceptable values are from ' + (0.1) + ' (s) to ' + maxDur + ' (s)');
    ...
    $("#duration_form").dialog(
        {
            ...
            buttons: [
...

                {
                    ...
                    , click: function (event) {
                        var durVal = $("#durValue").val();
                        if (durVal == "") {
                            alert('You cannot enter an empty value for the duration.');
                            return;
                        }
                        if (isNaN(durVal)) {
                        ...
                        dur = parseFloat(durVal);

                        if (dur < 0.1 || dur > maxDur) {
                            alert('Acceptable values are from ' + (0.1) + '(s) to ' + maxDur + '(s)');
                            return;
                        }

                        title = $('#vidASXTitle').val();
            ...
            open: function () {
                ...
                //$('.ui-slider').css('background', 'white');
                //$('.ui-slider-range').css('background', 'white');

                //$('#slider-range').find('span').css('background', 'white');
                //$('.ui-dialog-buttonpane').style('text-align', 'center');
        ...
        if (!p) {
            ...
            if (dat) {
                ...
                for (i = 0; i < dat.length; i++) {
                    ...
                    var isUploads = (li.title.toLowerCase().indexOf('uploads') != -1);

                    img = document.createElement('img');
                    $(img).css('margin-right', '2px');
                    img.src = isUploads ? 'images/Folder-Movies-icon.png' : 'images/a_px.png';
                    if (isUploads) {
                        li.title = 'Videos uploaded using offline video encoding facility ...'
                        $(img).css('margin-bottom', '-4px').css('margin-top', '2px');
                    }
